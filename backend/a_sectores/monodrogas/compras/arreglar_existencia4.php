...
$sql6 = "SELECT sum(cantidad_salida) as cantidad_salida  FROM `existencia_arreglada` where cod_mercaderia = '$cod_mercaderia' and anio_lote = '$anio_lote' and mes_lote = '$mes_lote' and lote = '$lote'";
...
$cantidad_guardar = $cantidad + $cantidad_salida;
 ...
 echo $sql = "UPDATE `existencia_arreglada` SET `cantidad_salida` = '$cantidad_guardar'  WHERE cod_mercaderia = '$cod_mercaderia' and lote = '$lote' and mes_lote = '$mes_lote' and anio_lote= '$anio_lote' and nro_factura = '$nro_factura'";
mysql_query($sql);
