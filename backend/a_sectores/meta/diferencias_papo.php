...
$sql1="select * from tr_ventas_detalle where fecha between '2013-03-01' and '2013-03-31' and tipo_fact like '001' and gtin != '7795337910308' order by nro_factura desc, cod_mercaderia";
...
$grupo=strtoupper($result21->fields["grupo"]);
...
$sql21="select * from tr_ventas_encabezado where nro_factura = $nro_factura";
$result21 = $db->Execute($sql21);
 $cod_movimiento=strtoupper($result21->fields["cod_movimiento"]);


$documento=strtoupper($result1->fields["documento"]);
$cantidad_programa=strtoupper($result1->fields["cantidad"]);
$nro_fact=strtoupper($result1->fields["nro_fact"]);
$precio_programa=strtoupper($result1->fields["precio_unitario"]);
$fecha_factura=strtoupper($result1->fields["fecha_factura"]);

$sql21="select * from archivo_meta  where nro_factura = $nro_factura and troquel = $troquel";
$result21 = $db->Execute($sql21);
$cantidad=strtoupper($result21->fields["cantidad"]);
$precio_unitario=strtoupper($result21->fields["precio_unitario"]);
$nro_fact=strtoupper($result21->fields["nro_factura"]);

if ($grupo != 3){
if ($nro_fact == ""){
$sql10 = "INSERT INTO archivo_papo (`mes`, `anio`, `nro_archivo`, `nro_factura`, `cod_mercaderia`, `troquel`, `nombre_comercial`, `cod_droga`, `drogas`, `cantidad`, `precio_unitario`, `documento`) VALUES ('$mes', '$anio', '$nro_archivo', '$nro_factura', '$cod_mercaderia', '$troquel', '$nombre_comercial', '$cod_droga', '$drogas', '$cantidad', '$precio_conv', '$documento');";
//$result10 = $db->Execute($sql10);

 
$pdf->Cell(20,5,$cod_movimiento,1,0,'L'); 
$pdf->Cell(20,5,$nro_factura,1,0,'L'); 	  
$pdf->Cell(30,5,$cod_mercaderia,1,0,'L'); 
$pdf->Cell(20,5,$troquel,1,0,'L'); 
$pdf->Cell(50,5,$nombre_comercial,1,0,'L'); 
$pdf->Cell(25,5,$presentacion,1,0,'L'); 
$pdf->Cell(25,5,$fecha_factura,1,0,'L'); 
$pdf->Cell(20,5,$grupo,1,0,'C'); 
