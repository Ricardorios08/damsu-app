...
$hoy = date("d-m-Y");
 ...
 $sql="select * from  tr_ventas_encabezado where (fecha between '2013-01-01' and '2013-12-31' and cod_diagnostico = 'C50' and documento > 5000) OR (fecha between '2013-01-01' and '2013-12-31' and cod_diagnostico = 'D05' and documento > 5000) group by documento order by departamento, denominacion";
$result = $db->Execute($sql);
...
<table width="100%" border="1" cellspacing="0">
  ...
  <tr bordercolor="#0066FF" bgcolor="#FF0000">
    ...
    <td bgcolor="#F0F0F0"><font size="3" face="Trebuchet MS"><strong>CANCER DE MAMA </strong></font></td>
    <td bgcolor="#F0F0F0"><div align="center"><font size="3" face="Trebuchet MS"><strong>A&Ntilde;O 2013 </strong></font></div></td>
 ...
 <td width="13%" bgcolor="#F0F0F0"><div align="center"><font color="#000000" face="Arial, Helvetica, sans-serif"><font size="2">Localidad </font></font></div></td>
...
$fecha_ingreso=strtoupper($result2->fields["fecha_ingreso"]);

if (($fecha_ingreso > '2012-12-31') and ($fecha_ingreso < '2014-01-01')){
...
$estadio=strtoupper($result->fields["estado"]);
 ...
 $sql2="select count(documento) as radio from  prestaciones_pacientes  where documento = $documento and cod_prestacion = 'TECO' and fecha_prestacion  between '2013-01-01' and '2013-12-31'";
$result2 = $db->Execute($sql2);
$radio=strtoupper($result2->fields["radio"]);
 
if ($radio == 0){$radio = "";}

  $sql2="select sum(neto) as gastado from  tr_ventas_encabezado where documento = $documento and fecha between '2013-01-01' and '2013-12-31'";
$result2 = $db->Execute($sql2);
$gastado=$result2->fields["gastado"];

$sql2="select count(nro_factura) as cantidad_atenciones from  tr_ventas_encabezado where documento = $documento and fecha between '2013-01-01' and '2013-12-31'";
$result2 = $db->Execute($sql2);
