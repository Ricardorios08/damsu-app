...
$server->register('entregas',array('variable1' => 'xsd:string' , 'variable2' => 'xsd:string'),array('return' => 'xsd:string'),$ns);
$server->register('entregas_detalle',array('variable1' => 'xsd:string' , 'variable2' => 'xsd:string'),array('return' => 'xsd:string'),$ns);
...
include ("../../conexiones/config.inc_papo.php");
$sql1=$sql;
mysql_query($sql1);
return new soapval('return','xsd:string',$sql1);
}

function entregas($variable1){
include ("../../conexiones/config.inc_papo.php");

$sql2="select * from tr_ventas_encabezado order by nro_factura desc";
$result2 = $db->Execute($sql2);
$nro_factura=$result2->fields["nro_factura"];


return new soapval('return','xsd:string',$nro_factura);
}

function entregas_detalle($variable1){
include ("../../conexiones/config.inc_papo.php");

$sql2="select * from tr_ventas_detalle order by nro_factura desc";
$result2 = $db->Execute($sql2);
$nro_factura=$result2->fields["nro_factura"];
