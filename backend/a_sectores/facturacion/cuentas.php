<?php 



$precio_actualizado = round($precio_actualizado,2);



if ($cantidad == ""){
$leyenda = "NO INGRESO CANTIDAD";
include ("../../../alertas/campo_vacio.php");
exit;
}

if ($cod_mercaderia== ""){
$leyenda = "NO INGRESO MERCADERIA";
include ("../../../alertas/campo_vacio.php");
exit;
}


$total = round($precio_actualizado * $cantidad);
//$total = $cantidad * $precio_unitario;

$sql9 = "SELECT count(*) as total FROM `ventas1_encab_temp`  WHERE  `nro_factura` = $nro_factura and tipo_fact = '$fact'";
$total_ordenes = $db->Execute($sql9);
$items=$total_ordenes->fields["total"];




if ($items < 18){


 if ($band== "SI"){


$sql = "INSERT INTO `ventas1_deta_temp` ( `tipo_fact` , `nro_factura` , `cod_detalle` , `cod_mercaderia` , `descripcion` , `presentacion` , `lote` , `mes_lote` , `anio_lote` , `cantidad` , `precio_unitario` , `total` , `proveedor`)  VALUES ('$tipo_fact' , '$nro_factura' , '' ,'$cod_mercaderia' , '$nombre_comercial', '$presentacion' , '$lote' , '$mes_lote' , '$anio_lote' , '$dif' , '$precio' , '$total' , '$proveedor')";
mysql_query($sql);}
 
else
	{ 
$sql = "INSERT INTO `ventas1_deta_temp` ( `tipo_fact` , `nro_factura` , `cod_detalle` , `cod_mercaderia` , `descripcion` , `presentacion` , `lote` , `mes_lote` , `anio_lote` , `cantidad` , `precio_unitario` , `total` , `proveedor` , `operador`)  VALUES ('$tipo_fact' ,'$nro_factura' , '' ,'$cod_mercaderia' , '$nombre_comercial', '$presentacion' , '$lote' , '$mes_lote' , '$anio_lote' , '$cantidad' , '$precio' , '$total' , '$proveedor' , '$operador')";
mysql_query($sql);
	}


}
else{
	echo "CANTIDAD DE ITEMS COMPLETOS, POR FAVOR PROCEDA A FACTURA";

}




?>