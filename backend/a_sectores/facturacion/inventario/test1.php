...
$result = $db->Execute($sql8);

$gt=$result->fields["gtin"];
...
ECHO $sql = "UPDATE `tr_ventas_detalle` SET `trazabilidad` = '1' WHERE `cod_detalle` = '$cod_detalle'";
mysql_query($sql);



  $sql = "INSERT INTO a_trazados (`tipo_fact`, `nro_factura`, `cod_detalle`, `cod_mercaderia`, `descripcion`, `presentacion`, `lote`, `mes_lote`, `anio_lote`, `cantidad`, `precio_unitario`, `total`, `proveedor`, `operador`, `gtin`, `resultado`, `transaccion`, `nro_serie`, `devuelve`, `cod_droga`, `nombre_droga`, `documento`, `nro_os`, `manual`) VALUES ( 'x', '$operador', '$cod_detalle', '$cod_mercaderia', '$descripcion', '$presentacion', '$lote', '$mes_lote', '$anio_lote', '1', '$precio_unitario', '$precio_unitario', '200', '$operador', '$gtin', '$resultado', '$transaccion' , '$nro_serie`, '$devuelve', '$cod_droga'`, '$nombre_droga', '$documento'`, '$nro_os', `'$manual');";
//mysql_query($sql);

 include ("inventario_trazabilidad.php");
exit;
...
echo $response = SendMedicamentos($args,$user,$pass);
...
?><table width="800" border="0" cellspacing="0">
  ...
  <tr bgcolor="#000099">
    ...
    <td width="528" bgcolor="#EDEDED">

<?PHP $errores = $response['errores'];
print_r($errores);
$i=0;
...
if ($resultado == "true"){
echo $sql = "INSERT INTO a_trazados (`tipo_fact`, `nro_factura`, `cod_detalle`, `cod_mercaderia`, `descripcion`, `presentacion`, `lote`, `mes_lote`, `anio_lote`, `cantidad`, `precio_unitario`, `total`, `proveedor`, `operador`, `gtin`, `resultado`, `transaccion`, `nro_serie`, `devuelve`, `cod_droga`, `nombre_droga`, `documento`, `nro_os`, `manual`) VALUES ( 'x', '$operador', '$cod_detalle', '$cod_mercaderia', '$descripcion', '$presentacion', '$lote', '$mes_lote', '$anio_lote', '1', '$precio_unitario', '$precio_unitario', '200', '$operador', '$gtin', '$resultado', '$transaccion' , '$nro_serie`, '$devuelve', '$cod_droga'`, '$nombre_droga', '$documento'`, '$nro_os', `'$manual');";
mysql_query($sql);
