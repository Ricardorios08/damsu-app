...
$sql = "DELETE FROM `ex_stock_temp_exis`";
mysql_query($sql);

$sql = "DELETE FROM `ex_stock_temp_provisorio`";
mysql_query($sql);

$tr_stock = "tr_stock_30-11-2012";

 $sql = "INSERT into ex_stock_temp SELECT * FROM `tr_stock` where fecha > '$fecha_anterior'";
mysql_query($sql);

  $sql="select * from ex_stock_temp group by cod_mercaderia order by cod_mercaderia, laboratorio";
...
$conta = $conta + 1;
  $sql1="select * from ex_stock_temp where cod_mercaderia = $cod_mercaderia group by laboratorio order by laboratorio";
...
$cant = $cant + 1;
    $sql2="select sum(cantidad) as ingresada  from ex_stock_temp where cod_mercaderia = $cod_mercaderia and laboratorio = $laboratorio and cod_movimiento = 1 and fecha > '$fecha_anterior'";
$result2 = $db->Execute($sql2);
$ingresada=strtoupper($result2->fields["ingresada"]);

   $sql2="select sum(cantidad) as salida  from ex_stock_temp where cod_mercaderia = $cod_mercaderia and laboratorio = $laboratorio and cod_movimiento = 6 and fecha > '$fecha_anterior'";
$result2 = $db->Execute($sql2);
$salida=strtoupper($result2->fields["salida"]);


   $sql2="select * from ex_stock_temp_provisorio where cod_mercaderia = $cod_mercaderia and laboratorio = $laboratorio  and mes = '$mes_anterior' and anio = $anio_anterior";
$result2 = $db->Execute($sql2);
$anterior=strtoupper($result2->fields["anterior"]);


  $sql2="select sum(precio_unitario) as precio_anterior  from ex_stock_temp_provisorio where cod_mercaderia = $cod_mercaderia and laboratorio = $laboratorio  and mes = '$mes_anterior' and anio = $anio_anterior";
$result2 = $db->Execute($sql2);
$precio_anterior=strtoupper($result2->fields["precio_anterior"]);



  $sql2="select sum(precio_unitario) as precio_ingreso  from ex_stock_temp where cod_mercaderia = $cod_mercaderia and laboratorio = $laboratorio and cod_movimiento = 1 and fecha  > '$fecha_anterior'";
$result2 = $db->Execute($sql2);
$precio_ingreso=strtoupper($result2->fields["precio_ingreso"]);

 $sql2="select sum(precio_unitario) as precio_egreso  from ex_stock_temp where cod_mercaderia = $cod_mercaderia and laboratorio = $laboratorio and cod_movimiento = 6 and fecha  > '$fecha_anterior'";
$result2 = $db->Execute($sql2);
