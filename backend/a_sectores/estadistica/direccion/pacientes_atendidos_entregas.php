...
/*
 ...
 $sql7="select count(documento) as cantidad, departamento from `est_departamento` group by departamento";
$result7 = $db->Execute($sql7);

  if (!$result7) die("fallo".$db->ErrorMsg());
  while (!$result7->EOF) {

echo $departamento=strtoupper($result7->fields["departamento"]); // numero
echo " - ";
echo $cantidad=strtoupper($result7->fields["cantidad"]); // numero
echo "<br>";
...
echo "<br>";
 $sql7="select count(documento) as cantidad, departamento from `est_departamento` group by fecha";
$result7 = $db->Execute($sql7);

  if (!$result7) die("fallo".$db->ErrorMsg());
  while (!$result7->EOF) {

echo $fecha=strtoupper($result7->fields["fecha"]); // numero.
echo " - ";
echo $departamento=strtoupper($result7->fields["departamento"]); // numero
echo " - ";
echo $cantidad=strtoupper($result7->fields["cantidad"]); // numero
...
$sql7="select departamento from `est_departamento` where provincia = 'MENDOZA' group by departamento";
$result7 = $db->Execute($sql7);
