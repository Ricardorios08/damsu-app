...
$fecha_nac_desde = $anio1."-01-01";
$fecha_nac_hasta = $anio."-12-31";

$a = "estudios_menores_15.xls";
...
?>
 ...
 <table width="1079" border="1" cellspacing="0">
  <tr bgcolor="#CCCCCC">
    <td colspan="14"><div align="center" class="Estilo2 Estilo8">ESTUDIOS REALIZADOS EN NIÑOS MENORES DE 15 AÑOS.  DURANTE EL: <?php echo $anio;?> </div></td>
  </tr>
  <tr bgcolor="#0000FF">
...
$hasta1 = $anio."-12-31";
   ...
   $sql="TRUNCATE est_prestaciones_pacientes";
$result = $db->Execute($sql);

   $sql="SELECT prestaciones_pacientes.cod_prestacion, pacientes.documento, pacientes.apellido, pacientes.nombre, pacientes.fecha_nac, prestaciones_pacientes.fecha_prestacion, prestaciones_pacientes.cant_realizado  FROM prestaciones_pacientes INNER JOIN pacientes ON prestaciones_pacientes.documento = pacientes.documento where prestaciones_pacientes.fecha_prestacion between '$desde1' and '$hasta1' and pacientes.fecha_nac between  '$fecha_nac_desde' and '$fecha_nac_hasta'  order by cod_prestacion, fecha_prestacion";
$result = $db->Execute($sql);
...
$cant_realizado=$result->fields["cant_realizado"];
  ...
  $sql3 = "INSERT INTO `est_prestaciones_pacientes` (`cod_prestacion`, `documento`, `apellido`, `nombre`, `fecha_nac`, `fecha_prestacion` , `cant_realizado`) VALUES ('$cod_prestacion', '$documento', '$apellido', '$nombre', '$fecha_nac', '$fecha_prestacion' , '$cant_realizado')";
...
$sql="select * from `est_prestaciones_pacientes` where fecha_prestacion between '$desde1' and '$hasta1' and cod_prestacion not like '0' group by cod_prestacion ";
	$result = $db->Execute($sql);
