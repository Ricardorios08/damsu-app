...
<table width="800" border="1" cellpadding="0" cellspacing="0" bgcolor="#EDEDED">
  <tr>
    <td colspan="8" bgcolor="#FFFFFF"><div align="center"><span class="Estilo14">CONSUMIMO POR PROVEEDOR  DESDE EL:<?PHP ECHO $fecha_d1;?>  HASTA <?PHP ECHO $fecha_h1;?>   </span></div></td>
  </tr>
  <tr>
...
<td width="347" bgcolor="#FFFFFF"><div align="center" class="Estilo77">Nombre</div></td>
   ...
   <td width="25" bgcolor="#FFFFFF"><div align="center"><span class="Estilo77">CONSUMIDOS</span></div></td>
...
//$sql="select * from tr_stock where fecha between '$fecha_d' and '$fecha_h' and laboratorio = '$laboratorios' group by cod_mercaderia ORDER BY fecha, cod_mercaderia";
     ...
     $sql="select * from tr_stock  where cuenta = '$laboratorios' AND fecha between '$fecha_d' and '$fecha_h' and cod_movimiento = 6 group by cod_mercaderia ORDER BY drogas, fecha, cod_mercaderia";
...
$cod_mercaderia=$result->fields["cod_mercaderia"];
   ...
   $sql="select sum(cantidad) as ingreso from tr_stock where fecha < '20$fecha_d'  and cuenta = '$laboratorios' and cod_mercaderia = $cod_mercaderia and cod_movimiento = 1";
$result2 = $db->Execute($sql);
 $tr_ingreso=$result2->fields["ingreso"];

   $sql="select sum(cantidad) as egreso from tr_stock where fecha < '20$fecha_d'  and cuenta = '$laboratorios' and cod_mercaderia = $cod_mercaderia and cod_movimiento = 6";
$result2 = $db->Execute($sql);
...
$saldo_anterior = $tr_ingreso - $tr_egreso;
  ...
  $sql="select sum(cantidad) as ingreso from tr_stock where fecha between '$fecha_d' and '$fecha_h' and cuenta = '$laboratorios' and cod_mercaderia = $cod_mercaderia and cod_movimiento = 1";
$result2 = $db->Execute($sql);
 $ingreso=$result2->fields["ingreso"];


    $sql="select sum(cantidad) as egreso from tr_stock where fecha between '$fecha_d' and '$fecha_h' and cuenta = '$laboratorios' and cod_mercaderia = $cod_mercaderia and cod_movimiento = 6";
$result2 = $db->Execute($sql);
