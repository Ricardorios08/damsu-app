...
$fecha=strtoupper($result3->fields["fecha"]);
 ...
 $sql = "SELECT * FROM `un_ventas1_deta_temp`  WHERE  `nro_factura` = $nro_factura and tipo_fact = '$tipo_fact'";
$result3 = $db->Execute($sql);
...
echo "-----".$total = round($cantidad * $precio_actualizado,2);
echo "<br>";
...
$cont = $cont + 1;
 ...
 echo $sql = "UPDATE `existencias` SET `cantidad_salida` = '$cantidad_existente' WHERE cod_mercaderia = $cod_mercaderia and mes_lote = '$mes_lote' and anio_lote = '$anio_lote' and lote = '$lote'";
//mysql_query($sql);

echo $sql = "INSERT INTO `stock` ( `cod_mercaderia` , `fecha` ,  `tipo_fact` , `cod_movimiento` , `nro_comprobante` , `cantidad` , `precio_unitario` , `lote` , `mes_lote` , `anio_lote` , `cuenta` ,  `tipo_cuenta` , `cod_operacion`  , `observaciones`  , `documento` , `cod_droga` ) VALUES ('$cod_mercaderia' , '$fecha' , '$tipo_fact' , '6' , '$nro_factura' , '$cantidad' , '$total' , '$lote' , '$mes_lote' , '$anio_lote' , '$proveedor' ,  '$tipo_doc' , '' , '' , '$documento' , '$cod_droga')" ;
//mysql_query($sql);


echo $sql = "INSERT INTO `ventas_detalle` ( `nro_factura` , `cod_detalle` , `cod_mercaderia` , `descripcion` , `presentacion` , `lote` , `mes_lote` , `anio_lote` , `cantidad` , `precio_unitario` , `total` , `tipo_fact` )  VALUES ('$nro_factura' , '' ,'$cod_mercaderia' , '$descripcion', '$presentacion' , '$lote' , '$mes_lote' , '$anio_lote' , '$cantidad' , '$total' , '$total' , '$tipo_fact' )";
//mysql_query($sql);
...
echo $sql = "INSERT INTO `ventas_encabezado` ( `tipo_fact` , `nro_factura` , `cod_operacion` , `tipo` , `nro_cliente` , `nro_cuenta` , `plan` , `operador` , `denominacion` , `fecha` , `bruto` , `descuento` , `iva` , `retencion` , `neto` , `forma_pago` ) VALUES ( '$tipo_fact' , '$nro_factura' , '1' , '' , '$documento' , '$documento' , '' , '$operador' , '$nombre_completo' , '$fecha' , '$total' ,  '' , '' , '' , '$total_factura' , '' )";
//mysql_query($sql);
