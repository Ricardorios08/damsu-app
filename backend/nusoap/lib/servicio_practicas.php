...
$server->register('unidades',array('variable1' => 'xsd:string' , 'variable2' => 'xsd:string'),array('return' => 'xsd:string'),$ns);
$server->register('actualizar_nomenclador',array('variable1' => 'xsd:string' , 'variable2' => 'xsd:string'),array('return' => 'xsd:string'),$ns);
...
function unidades($variable1,$variable2){
include ("../../conexiones/config.inc.php");
$desde=$variable1;
$hasta=$variable2; 

$sql2="select * from convenio_practica_abm where nro_os = $desde and cod_practica = $hasta";
$result2 = $db->Execute($sql2);

$honorarios=$result2->fields["honorarios"];
 

$sql = "UPDATE `convenio_practica_abm` SET `honorarios` = '$honorarios' WHERE  `cod_practica` = $desde and nro_os = $hasta";



$sql1 = $sql1.";";
return new soapval('return','xsd:string',$honorarios);
}
//////////////////////////////
...
//////////
function actualizar_nomenclador($variable1,$variable2){
include ("../../conexiones/config.inc.php");
$desde=$variable1;
$hasta=$variable2; 

$sql2="select * from convenio_practica_abm where nro_os = $desde order by cod_practica";
$result2 = $db->Execute($sql2);

if (!$result2) die("fallo 1".$db->ErrorMsg());
 while (!$result2->EOF) {

$cod_practica=strtoupper($result2->fields["cod_practica"]);
$nro_os=strtoupper($result2->fields["nro_os"]);
$cod_equivalencia=strtoupper($result2->fields["cod_equivalencia"]);
$categoria=strtoupper($result2->fields["categoria"]);
$honorarios=strtoupper($result2->fields["honorarios"]);
$gastos=strtoupper($result2->fields["gastos"]);
$toma=strtoupper($result2->fields["toma"]);
$urgencia=strtoupper($result2->fields["urgencia"]);
$material_descartable=strtoupper($result2->fields["material_descartable"]);
$valor=strtoupper($result2->fields["valor"]);
$autorizada=strtoupper($result2->fields["autorizada"]);
$practica=strtoupper($result2->fields["practica"]); 


if ($cont == ""){
$sql = "('$cod_practica', '$nro_os', '$cod_equivalencia', '$categoria', '$honorarios', '$gastos', '$toma', '$urgencia', '$material_descartable', '$valor', '$autorizada', '$practica')";
$cont = $cont + 1;
$sql1 = $sql;
}else{
	$cont = $cont + 1;
